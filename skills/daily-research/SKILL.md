...
### 4. 出力先
- **日次デルタレポート** → ローカル + Google Drive `daily-delta/` フォルダ
- **隔週サマリーレポート** → ローカル + Google Drive `biweekly-summary/` フォルダ
- **ステートファイル** → ローカルGitリポジトリで管理、毎実行後にpush
...
- **Google Drive MCP**（任意）: レポートのDriveアップロード。未接続でもローカル保存で動作する
...
Google Drive MCPが接続されていれば `daily-delta/` フォルダにアップロード。
未接続 → ローカル保存のみ。

### Step 6: ステート保存 + Git Push

1. `known-items.json` を保存
2. Bashで Git commit & push:
```bash
cd /path/to/org-glass-market
...
`biweekly-summary/` フォルダにアップロード（MCP接続時）。

### Step 4: ステート更新 + Git Push
1. `last_biweekly` を今日に更新
2. Git commit & push
...
├── reports/
│   ├── daily/                    # → GDrive daily-delta/
│   ├── biweekly/                 # → GDrive biweekly-summary/
│   └── state/known-items.json    # ★ Git管理
└── .gitignore
```
